...
const MessageContainer = () => {
    ...
    const { selectedChatType, selectedChatData, userInfo, selectedChatMessages, setSelectedChatMessage, setFileDownloadProgress, setIsDownloading } = useAppStore();
                ...
                {message.messageType === "text" && (
                    <div
                        className={`${message.sender._id === userInfo.id
                            ? "bg-[#1726ff] text-white border-[#8417ff]/50 "
                            : "bg-[#329115] text-white border-[#2feb38]/50 "
                            } border inline-block p-3 rounded my-1 max-w-[50%] break-words ml-9`} >
                        {message.content}
                    </div>
                )}
                {
            ...
            <div ref={scrollRef} >
                {
                    showImage && <div className="fixed z-[1000] top-0 left-0 h-[100vh] w-[100vw] flex items-center justify-center backdrop-blur-lg flex-col" >
                        ...
                        <div className="flex gap-5 fixed top-0 mt-5">
                            <button className="bg-black/20 p-3 text-2xl rounded-full hover:bg-black/50 cursor-pointer transition-all duration-300" onClick={() => dowloadFile(imageURL)}  >
                                <IoMdArrowRoundDown />
                            </button>
